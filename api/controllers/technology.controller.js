...
const { SuccessArrayResponse } = require("../utils/Responses/Succes.response");
...
const technologyController = {
  ...
  getAll: async (req, res, next) => {
    ...
    res.status(200).json(new SuccessArrayResponse(technologies));
  },
  getById: async (req, res, next) => {
    const id = req.params.id;
    const technology = await technologyService.getById(id);
    res.status(200).json(new SuccessResponse(technology));
  },
